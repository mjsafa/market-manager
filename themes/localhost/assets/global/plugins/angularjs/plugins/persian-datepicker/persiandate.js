...
// Converts two digit year to four digit
        ...
        this.gregorianDate_to_persianDateArray = function (date) {
            var tomorrow1 = (new Date(date.getFullYear(), date.getMonth(), date.getDate() + 1, 1, 0, 0));
            var tomorrow0 = (new Date(date.getFullYear(), date.getMonth(), date.getDate() + 1, 0, 0, 0));

            var dif = tomorrow0.getTime() - date.getTime();
            dif = dif / (1000 * 60 * 60 );
            if (dif == 0) {
                date = tomorrow1;
            }
        ...
        this.persian_to_gregorian_Date = function (year, month, day) {
            ...
            var time1 = new Date(greg[0], greg[1] - 1, greg[2], 0, 0, 0);
            var time2 = new Date(greg[0], greg[1] - 1, time1.getDate() + 1, 0, 0, 0);
            return new Date(greg[0], greg[1] - 1, greg[2], 0, 0, 0);
        };
