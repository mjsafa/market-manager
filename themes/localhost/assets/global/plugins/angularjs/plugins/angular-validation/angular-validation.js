      ...
      link: function(scope, elm, attrs, ctrl) {
        ...
        var updateErrorMsg = function(message, isFieldValid) {
          ...
          if(hasValidation && !isFieldValid && ctrl.$dirty) {
            ...
            (errorElm.length > 0) ? errorElm.text(message) :  ( elm.parent('.input-group').length > 0) ? elm.parent('.input-group').after('<span class="validation validation-'+elmInputName+' text-danger">'+message+'</span>') : elm.after('<span class="validation validation-'+elmInputName+' text-danger">'+message+'</span>');
          }else {
