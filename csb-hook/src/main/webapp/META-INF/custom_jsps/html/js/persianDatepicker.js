    ...
    var persianDatepicker = (function () {
        function persianDatepicker(element, userOptions) {
            var defaults = {
                ...
                onHide: function () { },
                onSelect: function () { },
                onSelectFunction : function(jDate, gDate){},
            ...
            if(self.options.selectedGregorianDate && !self.options.selectedDate){
                var gDay = parseInt(self.options.selectedGregorianDate.split("/")[2]);
                var gMonth = parseInt(self.options.selectedGregorianDate.split("/")[1]);
                var gYear = parseInt(self.options.selectedGregorianDate.split("/")[0]);

                var jDateArray = JalaliDate.gregorianToJalali(gYear, gMonth, gDay);

                self.options.selectedBefore = true;
                jDay = parseInt(jDateArray[1]) + 1;
                self.options.selectedDate = jDateArray[0] + "/" + jDay + "/" + jDateArray[2];
            }
        ...
        persianDatepicker.prototype = {
            ...
            showDate: function (el, v) {
                ...
                self.persianDate = self.persianDate.parse(v);
                el.data('date', { jDate: v, gDate: this.jDateFunctions.getGDate(this.persianDate)._toString("YYYY/MM/DD/DW") });
                self.options.onSelectFunction(v, this.jDateFunctions.getGDate(self.persianDate));
