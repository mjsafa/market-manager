<%@ page import="com.liferay.portal.kernel.util.StreamUtil" %>
...
<liferay-util:buffer var="html">
    <liferay-util:include
        ...
        page="/html/taglib/ui/input_localized/page.portal.jsp"
        useCustomPage="<%= false %>"
    />
</liferay-util:buffer>
...
if(null == placeHolder  || placeHolder.trim().equals("")){
    placeHolder = "";
}
...
<span class="input-localized input-localized-<%= type %>" id="<portlet:namespace /><%= id %>BoundingBox">
	<c:choose>
		...
		<c:when test='<%= type.equals("input") %>'>
			<input placeholder='<liferay-ui:message key="<%= placeHolder %>" />' class="language-value <%= cssClass %>" dir="<%= mainLanguageDir %>" <%= disabled ? "disabled=\"disabled\"" : "" %> id="<portlet:namespace /><%= HtmlUtil.escapeAttribute(id + fieldSuffix) %>" name="<portlet:namespace /><%= HtmlUtil.escapeAttribute(name + fieldSuffix) %>" type="text" value="<%= HtmlUtil.escapeAttribute(mainLanguageValue) %>" <%= InlineUtil.buildDynamicAttributes(dynamicAttributes) %> />
		</c:when>
