		...
		<c:otherwise>
            ...
            <div class="input-group big">
                ...
                <span class="twitter-typeahead">
                    ...
                    <input class="form-control persian-date" <%= disabled ? "disabled=\"disabled\"" : "" %> placeholder="<%= StringUtil.toLowerCase(simpleDateFormatPattern) %>" type="text"  >
                </span>

            </div>

			<input style="display: none;" class="input-medium" <%= disabled ? "disabled=\"disabled\"" : "" %> id="<%= namespace + name %>" name="<%= namespace + name %>" placeholder="<%= StringUtil.toLowerCase(simpleDateFormatPattern) %>" type="text" value="<%= format.format(calendar.getTime()) %>" />
		</c:otherwise>
	</c:choose>
...
<aui:script use='<%= "aui-datepicker" + (BrowserSnifferUtil.isMobile(request) ? "-native" : StringPool.BLANK) %>'>
    ...
    window.initiateDatePicker = function(){
        $(".persian-date").persianDatepicker({
            formatDate: "YYYY/MM/DD",
            cellWidth: 40,
            cellHeight:40,
            selectedGregorianDate:"<%=yearValue%>/<%=monthValue%>/<%=dayValue%>",
            onSelectFunction: function(jDate, gDate){
                var date = gDate.getDate();
                var month = gDate.getMonth();
                var year = gDate.getFullYear();
                $('#<%= dayParamId %>').val(date);
                $('#<%= monthParamId %>').val(month);
                $('#<%= yearParamId %>').val(year);
            }
        });
    }
