...
<%
    ...
    String templateEscapeName = HtmlUtil.escape("templateId");
    List<Template> templates = TemplateLocalServiceUtil.getTemplatesByCompany(themeDisplay.getCompanyId());
...
<div class="aui-helper-hidden" id="<portlet:namespace />addLayout">
    ...
    <aui:form action="<%= editPageURL %>" enctype="multipart/form-data" method="post" name="fm2">
        ...
        <aui:input id="addLayoutRedirect" name="redirect" type="hidden"
                   value='<%= HttpUtil.addParameter(redirectURL.toString(), liferayPortletResponse.getNamespace() + "selPlid", selPlid) %>'/>
        ...
        <aui:fieldset>
            ...
            <%--  SELECT TEMPLATE --%>
            <input name="<portlet:namespace />ExpandoAttributeName--<%= templateEscapeName %>--" type="hidden"
                   value="<%= templateEscapeName %>"/>
            <select id="<%= randomNamespace %><%= templateEscapeName %>"
                    name="<portlet:namespace />ExpandoAttribute--<%= templateEscapeName %>--">
            ...
            <c:if test="<%= !layoutPrototypes.isEmpty() %>">
                <aui:select label="template" name="layoutPrototypeId" showEmptyOption="<%= true %>">
                    ...
                    <aui:option label="<%= HtmlUtil.escape(layoutPrototype.getName(user.getLanguageId())) %>"
                                value="<%= layoutPrototype.getLayoutPrototypeId() %>"/>
            ...
            <div id="<portlet:namespace />hiddenFields">
                <aui:select id="addLayoutType" name="type">
                    ...
                    <aui:option
                            disabled="<%= firstLayout && !PortalUtil.isLayoutFirstPageable(PropsValues.LAYOUT_TYPES[i]) %>"
                            label='<%= LanguageUtil.get(pageContext, "layout.types." + PropsValues.LAYOUT_TYPES[i]) %>'
                            value="<%= PropsValues.LAYOUT_TYPES[i] %>"/>
            ...
            <div class="aui-helper-hidden" id="<portlet:namespace />layoutPrototypeLinkOptions">
                <aui:input label="automatically-apply-changes-done-to-the-page-template"
                           name="layoutPrototypeLinkEnabled" type="checkbox" value="true"/>
            </div>
        </aui:fieldset>
